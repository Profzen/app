...
- **7. Harmonisation Intégrale des Marges Hautes avec la Formule Paramètres & Fallback Web/iOS (68 / 68 Écrans)** :
  - **Standardisation de la Marge Haute (`safeArea`)** : Application de la formule d'espacement universelle de l'écran *Paramètres / Plus* (`paddingTop: Platform.OS === 'android' ? Math.max(StatusBar.currentHeight || 0, 44) + 6 : 14`) sur **l'intégralité des 68 écrans** de l'application.
  - **Prise en charge universelle Android, iOS et Web** : Garantit au moins 50px d'aération sur Android (s'adaptant à la hauteur réelle de la barre de statut de chaque téléphone) et un fallback propre de 14px à 24px sur Web et iOS pour que les en-têtes (ex: "Bonjour, David") ne soient plus jamais plaqués contre le haut de l'écran.
