...
- **8. Réordonnancement du Flux des Services (`ChooseServiceScreen.js`)** :
  - **Clarification du Parcours** : Le clic sur *"Pay bills and send essentials"* redirige vers l'écran intermédiaire **`ChooseServiceScreen.js`** qui affiche par défaut la grille des **5 types de services (Seconde Capture)** :
    1. *Eau, gaz, électricité*
    2. *Recharge mobile* -> Ouvre la sélection de l'opérateur (Première Capture / MTN Nigeria)
    3. *Internet, TV, Jeux & Crypto*
    4. *Envoyer/Demander des fonds* -> Redirige directement vers `SendMoneyScreen.js` ("Envoyer des fonds")
    5. *Produits & Services essentiels* -> Redirige vers `ShopsScreen.js`
  - **Navigation Fluide** : Chaque option de la grille redirige vers l'écran cible approprié et la flèche retour permet de revenir à la sélection générale.
