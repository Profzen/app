...
- **8. Réordonnancement du Flux des Services (`ChooseServiceScreen.js`)** :
  - **Clarification du Parcours** : Le clic sur *"Pay bills and send essentials"* redirige vers l'écran intermédiaire **`ChooseServiceScreen.js`** qui affiche par défaut la grille des **5 types de services** (*Eau/Gaz/Élec, Recharge mobile, Internet/TV, Envoyer des fonds, Produits essentiels*).
- **9. Intégration Complète du Parcours d'Envoi de Fonds en 2 Étapes (Captures Web)** :
  - **[`SendMoneyScreen.js`](file:///g:/zen/projets/DizzitApp/app/src/screens/SendMoneyScreen.js) (Étape 1: Montant - Capture 1)** : Implémentation du stepper d'étape 1, du sélecteur de devises (`Ar Ariary malgache`, `XOF`, `GHS`, `NGN`, `USDC`), de la carte de montant avec conversion `≈ 1,03 DZ`, du pavé numérique virtuel interactif et du bouton de continuation.
  - **[`SendMoneyMethodScreen.js`](file:///g:/zen/projets/DizzitApp/app/src/screens/SendMoneyMethodScreen.js) (Étape 2: Méthode - Capture 2)** : Implémentation de la sélection par bouton radio (*Dizzy - Portefeuille DIZY* vs *Mobile Money*), du bandeau de sécurité et du double bouton *"Précédent"* / *"Suivant"*.
...
- **7. Harmonisation Intégrale des Marges Hautes avec la Formule Paramètres & Fallback Web/iOS (68 / 68 Écrans)** :
  - **Standardisation de la Marge Haute (`safeArea`)** : Application de la formule d'espacement universelle de l'écran *Paramètres / Plus* (`paddingTop: Platform.OS === 'android' ? Math.max(StatusBar.currentHeight || 0, 44) + 6 : 14`) sur **la totalité absolue des 68 écrans** (y compris écrans d'authentification, modales, paramètres, tableaux de bord et tunnels de paiement).
  - **Prise en charge universelle Android, iOS et Web** : Audit exhaustif validant un espacement supérieur minimal de 50px sur Android et un fallback d'aération élégant de 14px à 24px sur iOS/Web pour que les en-têtes (ex: "Plus & Paramètres", "Bonjour, David") ne soient plus jamais plaqués contre le haut de l'écran.
